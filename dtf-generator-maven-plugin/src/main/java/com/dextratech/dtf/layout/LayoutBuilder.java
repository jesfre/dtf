...
public class LayoutBuilder extends DtfAbstractMojo {
	...
	public void execute() throws MojoExecutionException {
		...
		for (int count = 0; fileIt.hasNext(); count++) {
			...
			try {
				...
				DextraSystemLogger.error("Error building dataset layout: " + e.getMessage());
				e.printStackTrace();
			}
		}
	}
	...
	private void createExcelDatasourceLayout(String excelFileLocation) {
		try {
			...
			DextraSystemLogger.error("Cant generate Excel layout...");
		}
	}
	...
	private void parseAndCreateWilcards(File html) {
		...
		try {
			...
			DextraSystemLogger.println("---\tCOMMANDS\t---");
			for (NodeIterator it = nodeList.elements(); it.hasMoreNodes();) {
				...
				if (colCount == 3) {
					...
					DextraSystemLogger.print("\t");
					if (StringUtils.isNotBlank(command)) {
						DextraSystemLogger.print(command);
					}
					DextraSystemLogger.print(" | ");
					...
					if (StringUtils.isNotBlank(target)) {
						DextraSystemLogger.print(target);
					...
					if (StringUtils.isNotBlank(value)) {
						...
						DextraSystemLogger.print(value);
					}
	...
	private void escribeArchivo(String nombre, String ruta, String contenido) {
		...
		if (f.exists()) {
			f.delete();
			DextraSystemLogger.println("Se elimino el archivo");
		}
		...
		if (!f.exists()) {
			try {
				...
				DextraSystemLogger.println("Nueva plantilla en " + f.getAbsolutePath());
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
